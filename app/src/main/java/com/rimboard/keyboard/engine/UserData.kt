...
class UserData private constructor(dir: File) {
    ...
    fun glideCandidates(path: GlidePath, limit: Int): List<Triple<String, Int, Double>> =
        // The gate itself lives in [PersonalWords.fitting], which is where the
        // note about "wolfram" after "helo" went with it. It moved because
        // there are two personal lists now -- this one and the words declared
        // in Android's own dictionary -- and both a swipe and the strip have to
        // apply the same rule to both.
        com.rimboard.keyboard.model.PersonalWords.fitting(learned.keys, path, limit)
            .map { (w, cost) -> Triple(w, learned[w] ?: 0, cost) }
